# From https://github.com/bindestriche/srt_fix/
...
def dedupe_yt_srt(subs_iter):
    ...
    for subtitle in subs_iter:
        ...
        if (
            previous_subtitle is None
        ):  # first interation set previous subtitle for comparison
        ...
        if (
            subtitle.start - subtitle.end < timedelta(milliseconds=150)  # very short
            and subtitle.text in previous_subtitle.text
        ):  # same text as previous
        ...
        if current_lines[0] == last_lines[-1]:  # if first current is  last previous
            if len(last_lines) == 1:
                if (
                    len(last_lines[0].split(" ")) < 2 and len(last_lines[0]) > 2
                ):  # if  is just one word
                    singleword = True
                    subtitle.text = (
                        current_lines[0] + " " + "\n".join(current_lines[1:])
                    )  # remove line break after single word

                else:
                    subtitle.text = "\n".join(
                        current_lines[1:]
                    )  # discard first line of current
            else:
                subtitle.text = "\n".join(
                    current_lines[1:]
                )  # discard first line of current
